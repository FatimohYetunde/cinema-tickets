...
public class TicketServiceImpl extends TicketPaymentServiceImpl implements TicketService {
    ...
    private void validateTicketPurchase(TicketTypeRequest... typeRequests) throws InvalidPurchaseException {
        Arrays.stream(typeRequests).filter(x -> x.getTicketType().equals(TicketTypeRequest.Type.ADULT)).findFirst()
                .orElseThrow(() -> new InvalidPurchaseException(
                        "You can not purchase a ticket without purchasing an Adult ticket"));
    }
